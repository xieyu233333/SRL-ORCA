...
//-----------------------------------------------------------------------------
//	Note:   Interaction 6机器人ORCA运动--基础程序--cpp文件
//-----------------------------------------------------------------------------
...
#include "collvoid_local_planner/six_robotmove.h"
...
//                               { 0, 1, 2, 3, 4, 5 }};
//机器人初始坐标 行--x,y 列--机器人编号
float _beginloc_list[6][2] = {{ 2.5, 0 },
                                { 1.5, 3 },
                                { -1, 2.5 },
                                { -3, 0.5 },
                                { -2, -2 },
...
//速度列表 横--速度列表
//x向--5
float _speed_list[6] = {0.06, 0.10, 0.14 ,0.18, 0.22, 0.25};

//超参数
//机器人初始方向角
float _heading_list[6] = { 0, 0, 0, 3.14, 3.14, 3.14 };
...
void init()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        IDSI_Rank[i] = nh.subscribe<std_msgs::Float32MultiArray>("/tb3_" + std::to_string(i) +"/idsi_topic", 10, boost::bind(&IDSI_callback, _1, i));
    }
...
void local_compute()
{
    ...
    {
        //如果两机器人--均到达终点,
        if( (arrival_bool[1] == true)&& (arrival_bool[2] == true)&& (arrival_bool[3] == true)&& (arrival_bool[4] == true)&& (arrival_bool[5] == true) ) //(arrival_bool[0] == true) && 
        {
            //计算重新开始位置--机器人-0
            ...
            //计算重新开始位置--机器人-2
            restart_location_2.model_name = "tb3_" + std::to_string(2);
            ...
            //计算重新开始位置--机器人-3
            restart_location_3.model_name = "tb3_" + std::to_string(3);
            ...
            set_model_pub_[3].publish(restart_location_4);
            set_model_pub_[5].publish(restart_location_5);
...
void Print_IDSI()
{
  ...
  {
    // 打印接收到的数组
    ROS_INFO("Received Robot = %i, RobIDSI-Array  = [%f, %f, %f, %f, %f, %f, ]", i, idsi_data[i][0], idsi_data[i][1], idsi_data[i][2], idsi_data[i][3], idsi_data[i][4], idsi_data[i][5] );
  } 
}
