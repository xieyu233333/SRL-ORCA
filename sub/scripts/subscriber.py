...
def pos_callback(data, args):
    ...
    y_in = args[2]
    yaw = args[3]
    x_in[tbnum] = data.pose.pose.position.x
    y_in[tbnum] = data.pose.pose.position.y
    yaw[tbnum] = euler_form_quaternion(data.pose.pose.orientation)[2]
...
def listener(x_in, y_in,yaw):
    ...
    # 创建一个订阅者，订阅里程计信息
    rospy.Subscriber("/tb3_1/odom", Odometry, pos_callback, [0, x_in, y_in,yaw])
    rospy.Subscriber("/tb3_2/odom", Odometry, pos_callback, [1, x_in, y_in,yaw])
    rospy.Subscriber("/tb3_3/odom", Odometry, pos_callback, [2, x_in, y_in,yaw])
    rospy.Subscriber("/tb3_4/odom", Odometry, pos_callback, [3, x_in, y_in,yaw])
    rospy.Subscriber("/tb3_5/odom", Odometry, pos_callback, [4, x_in, y_in,yaw])
    # 循环等待回调函数
    # rospy.spin()
